...
export class AnalyticsEngine {
  ...
  #pairSnapshots = new Map<string, PairState[]>();
  #flows = new Map<string, FlowRow[]>();
  ...
  #updatePair(
    ...
    appendMapArray(this.#pairSnapshots, pair.pair, pair);
  ...
  #applySwap(event: SwapAnalyticsEvent, pair: PairState, blockNumber: bigint, blockHash: Hex, timestamp: number): void {
    ...
    appendMapArray(this.#flows, pair.pair, {
      pair: pair.pair,
      timestamp,
  ...
  #poolMetrics(pair: PairState, asOfTimestamp: number): PoolMetrics {
    ...
    const flows = timestampWindow(this.#flows.get(pair.pair) ?? [], cutoff, asOfTimestamp);
  ...
  #pairStateAt(pairId: string, timestamp: number): PairState | null {
    const snapshots = this.#pairSnapshots.get(pairId) ?? [];
    let low = 0;
    let high = snapshots.length;
    while (low < high) {
      const middle = Math.floor((low + high) / 2);
      if (snapshots[middle]!.updatedAtTimestamp <= timestamp) low = middle + 1;
      else high = middle;
    }
