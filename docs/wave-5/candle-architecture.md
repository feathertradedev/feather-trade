...
`PostgresAnalyticsStore` commits checkpoint metadata, the new canonical block, and only changed candle buckets in one append transaction. It never rewrites historical rows on the ordinary live path. Retained reorgs use a separate full-save path that replaces the canonical branch and candle set, deleting rows absent from the recovered checkpoint. Replay-event append and trimming use a second transaction after either canonical write, and the service does not advance or publish its live cursor until those replay rows are durable. A failed canonical write remains pending and must be repaired before another block can be accepted; an unrecoverable live-source failure closes the server. CI executes these paths against PostgreSQL and proves delta appends, fail-once repair, orphan deletion, mutable row replacement, replay trimming, and restart restoration.
